...
TEST_CASE("Test game master guess evaluation", "[evaluate_guess]") {
  std::vector<unsigned char> solution = {0,1,1,2,3};
  GameMaster master = GameMaster::with_solution(solution, 5);
  ...
  SECTION("Bad guess") {
    std::vector<unsigned char> guess = {4,4,4,4,4};
  ...
  SECTION("Ok guess") {
    std::vector<unsigned char> guess = {0,4,4,4,1};
    response r = master.evaluate_guess(guess);
    REQUIRE(r.perfect == 1);
